...
class SetExposureCompensation1Button extends StatelessWidget {
  ...
  const SetExposureCompensation1Button({
    Key? key,
    this.style,
    this.focusNode,
    this.autofocus = false,
    this.clipBehavior = Clip.none,
    this.label = "Set Exposure Compensation to 1",
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return OutlinedButton(
        ...
        onPressed: () async {
          var response =
              await setOption(name: 'exposureCompensation', value: 1.0);
